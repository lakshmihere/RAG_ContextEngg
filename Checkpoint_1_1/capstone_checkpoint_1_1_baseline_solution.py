...
#
# This is the first checkpoint of your **capstone project**, which you build across the whole program. Here you establish the **no-retrieval baseline**: you test how a plain LLM behaves on your chosen 
# scenario *before* adding any retrieval, document where it falls short, and argue whether (and why) retrieval is needed.This mirrors **Lab 1.1** — where you probed an LLM for five failure modes — but 
# now applied to the realistic corpus you'll build your system on. The deliverable is a **400-550-word written analysis** (see the final section); this notebook is the tool that produces the evidence for 
# that writeup.
#
...
#
# **Important:** This checkpoint uses **no retrieval and no document parsing**. You are deliberately testing the LLM *without* access to your corpus, so you can see what it gets wrong. Loading and 
# indexing your documents comes in Checkpoint 2.1.
#
# %% [markdown]
...
def my_probe_prompts() -> list[str]:
    """Return 3-5 of YOUR OWN baseline prompts for the Wikipedia scenario."""
    return [
        # 1. Specific fact from one article
        ...
        "Wikipedia articles. Support the comparison with a direct quote "
        "from each article.",

        # 5. Specific organizational fact
        ...
        # 6.  Additional one on Steve Jobs 
        "According to the Wikipedia article on Steve Jobs, what does the opening "
        "paragraph say about his role in the personal computer revolution? "
        "Identify the specific roles and companies attributed to him."
    ]
...
def run_probes() -> None:
    check_api_key()
    chain = build_chain()
    prompts = my_probe_prompts() # Run ONLY your Step 3 probe prompts
    
    print(f"Capstone Checkpoint 1.1 — baseline (no retrieval)")
    print(f"Scenario: {SCENARIO}  |  Model: {LLM_MODEL}")
    print(f"\nRunning {len(prompts)} probe prompts...\n")
    print(f"Logging to: {LOG_PATH.name}\n")
    print("-" * 72)
    
    for i, question in enumerate(prompts, start=1):
        ...
        print(response)
        print("-" * 72)
        log_response(SCENARIO, question, response)
    print("Done. Now READ each response against the real document. For each, "
...
#---------------------- Please check the worksheet in the repository of the detailed evalution of the prompts for Step 5 ----------------------------------------------------------------

# %% [markdown]
# ## Step 5 — Your written analysis (the graded deliverable)  
# The checkpoint deliverable is a **500-750 word written submission**, not code. Using the evidence in `checkpoint_1_1_responses.log`, address each of these (this is the required structure 
# from the assignment):
#
