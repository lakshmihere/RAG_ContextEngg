...
# Read CSV
df = pd.read_csv("checkpoint_3_1_hybrid_tagged_baseline.csv")
data = df.to_dict(orient="records")
with open("checkpoint_3_1_hybrid_tagged_baseline.json", "w", encoding="utf-8") as f:
    json.dump(data, f, indent=2, ensure_ascii=False)
print(f"Converted {len(data)} rows.")
print("Saved as checkpoint_3_1_hybrid_tagged_baseline.json")
...
def main():
    in_path = (
        sys.argv[1]
        if len(sys.argv) > 1
        else "checkpoint_3_1_hybrid_tagged_baseline.json"
    )
    ...
    out_path = (
        ...
        if len(sys.argv) > 3
            ...
            Path(in_path).with_name(
                Path(in_path).stem + "_variants.json"
            )
        )
    )
    ...
    print(
        f"Generating up to {n} paraphrase(s) "
        f"for each of {len(data)} question(s)...\n"
    )
    ...
    for i, entry in enumerate(data, 1):
        ...
        # Keep original with normalized schema
        augmented.append({
    ...
    # Save AFTER all questions are processed
    ...
    print(
        f"Done: {len(data)} original question(s) + "
        f"{variant_count} generated variant(s) "
    ...
    print(
        "Each variant reuses its original's grading_notes, "
        "so the same answer should apply."
    )

    print("\nNext steps:")

    print(
        "  1. Open the file above and delete any variant "
        "whose meaning drifted."
    )

    print(
        "  2. Evaluate the expanded set with your "
        "Checkpoint 3.1 Wikipedia evaluation script:"
    )

    print(
        "       python "
        "capstone_checkpoint_3_1_repaired_LMStudio_v2.py "
        f"hybrid_tagged --inputs {out_path}"
    )

    print(
        "     Compare original vs variant performance "
        "to test robustness to rephrasing."
    )
